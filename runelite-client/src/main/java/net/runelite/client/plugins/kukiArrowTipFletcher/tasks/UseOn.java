...
public class UseOn implements ScriptTask {
	...
	@Override
	public int execute() {
		if (!Players.getLocal().isAnimating() && hasXpDrop() && !Production.isOpen() && Inventory.contains(52)) {
			Inventory.getFirst("Feather").useOn(Inventory.getFirst(52));
		} else if  (!Players.getLocal().isAnimating() && hasXpDrop() && !Production.isOpen() && Inventory.contains(40) && !Inventory.contains(52)) {
				Inventory.getFirst("Headless arrow").useOn(Inventory.getFirst(40));
			} else if (!Players.getLocal().isAnimating() && hasXpDrop() && !Production.isOpen() && Inventory.contains(41) && !Inventory.contains(52)) {
