package net.runelite.client.firemaker.tasks;
...
public class LightFire implements ScriptTask {
	...
	public int execute() {
		...
		if (Players.getLocal().isIdle()) {
			...
			boolean hasLogs = Inventory.contains(WILLOW_LOGS_ID);
			while (hasLogs) {
				Inventory.getFirst("Tinderbox").useOn(Inventory.getFirst("Willow logs"));
				...
				hasLogs = Inventory.contains(WILLOW_LOGS_ID);
			}
			return 1000;
		}
